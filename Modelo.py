...
class Procesador(object):
    def __init__(self):  
        self.camara = cv2.VideoCapture(0, cv2.CAP_DSHOW)                                                   # Se inicializan los atributos por defecto que tendra el modelo 
        self.frame = None
    ...
    def asignar_kernel(self, kernel):
        self.kernel = kernel

#%%     Camara
    def cargar_imagen(self, ruta_imagen, escala_grises=False):
        imagen = cv2.imread(ruta_imagen)
        imagen = cv2.rotate(imagen, cv2.ROTATE_90_COUNTERCLOCKWISE)
        imagen = cv2.flip(imagen, 0)
        
        if (escala_grises):
            self.frame = cv2.cvtColor(imagen, cv2.COLOR_BGR2GRAY)
        else:
    ...
    def recuperar_imagen_cargada(self):
        return self.frame

    def resetear_frame_procesado(self):
        self.frame_procesado = np.copy(self.frame)
        return True
    ...
    def apagar_camara(self):                                                                # Se apaga la camara y retorna un Falso
        if (self.camara.isOpened()):
            self.camara.release()
            print("\n la camara estaba abierta y se ha cerrado con exito")
...
#%%     Procesamiento de Frame
    ...
    def extraer_color(self, color):                                                         # Coloca en 0 los pixeles de los canales
        ...
        if (color == 'Rojo'):
            ...
            self.frame_procesado = cv2.cvtColor(self.frame_procesado, cv2.COLOR_BGR2GRAY)
    
    def ecualizar_color(self, color):                                                       # Ecualiza el histograma del color seleccionado                                                    
    
        if (color == 'Gris'):
            self.frame_procesado[:,:] = cv2.equalizeHist(self.frame_procesado[:,:])
            return
    ...
    def identificar_texto(self):
        # Gray Scale
        if (len(self.frame_procesado.shape) == 2):
            color = (0, 0, 0)
        # Color scale
        else:
            color = (255, 0, 0)
        
        # La imagen se transforma inicialmente debido a la orientación inducida por PyQt
        # por lo que se transforma la imagen a como estaba originalmente para aplicar OCR
        imagen = cv2.flip(self.frame_procesado, 0)
        imagen = cv2.rotate(imagen, cv2.ROTATE_90_CLOCKWISE)
        
        data = ocr.image_to_data(imagen, output_type=Output.DICT)
        
        n_boxes = len(data['text'])
        for i in range(n_boxes):
            # Se capturan las palabras con una confianza > 60%
            if int(data['conf'][i]) > 60:
                (x, y, w, h) = (data['left'][i], data['top'][i], data['width'][i], data['height'][i])
                imagen = cv2.rectangle(imagen, (x, y), (x + w, y + h), color, 1)
        
        imagen = cv2.rotate(imagen, cv2.ROTATE_90_COUNTERCLOCKWISE)
        imagen = cv2.flip(imagen, 0)
        
        self.frame_procesado = np.copy(imagen)
        del imagen
        return data['text']
