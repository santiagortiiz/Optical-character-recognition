...
from PyQt5.QtWidgets import QMainWindow, QGraphicsScene, QFileDialog;                       # Librerias necesarias para crear ventanas
...
class InterfazGrafica(QMainWindow):
    def __init__(self, ppal = None):
        ...
        # Banderas
        self.imagen_estatica_cargada = False
...
#%%     Espacio grafico del procesador de movimiento
        self.escena_4 = pg.ImageItem()                                                       
        escena_4 = QGraphicsScene()                                                            
        escena_4.addItem(self.escena_4)                                                        
        self.espacio_grafico_6.setScene(escena_4) 

        self.ocultar_componentes()                                     

#%%     CRONOMETRO
    def cronometro_on_off(self):
        if (self.imagen_estatica_cargada):
            if (self.cronometro.isActive()):
                self.cronometro.stop()
            else:
                self.cronometro.timeout.connect(self.recuperar_imagen)
                self.cronometro.start(30)

        if (self.camara_activada):
            if (self.cronometro.isActive()):
                self.cronometro.stop()
            else:
                self.cronometro.timeout.connect(self.recuperar_frame)
...
#%%     IMAGEN ESTATICA
    ...
    def cargar_imagen(self):
        ...
        escala_grises = False
        ruta_imagen, filtro = QFileDialog.getOpenFileName(self, "Cargar Imagen", "../Dataset", "Images (*.png *.jpg *.jpeg)")
        
        self.imagen_estatica_cargada = self.__controlador.cargar_imagen(ruta_imagen, escala_grises)
        
        if (self.imagen_estatica_cargada):
            self.mostrar_componentes()
            
            imagen_cargada = self.__controlador.recuperar_imagen_cargada()                    
            self.graficar_imagen_cargada(imagen_cargada)
            self.graficar_frame_procesado(imagen_cargada)
            
    def recuperar_imagen(self):
        if (self.imagen_estatica_cargada):
            self.__controlador.resetear_frame_procesado()  
            procesamientos_aplicados = self.procesar_frame()   
            self.determinar_ventana_actual(procesamientos_aplicados, None)
   
    def identificar_texto(self):                                                                   
        self.cronometro.stop()
        
        texto_identificado = self.__controlador.identificar_texto() 
        self.campo_ocr.clear()
        self.campo_ocr.setText(str(texto_identificado)) 
        
        imagen_procesada = self.__controlador.recuperar_frame_procesado()
        self.graficar_frame_procesado(imagen_procesada)
...
#%%     CAMARA
    ...
    def activar_camara(self):
        ...
        if (self.cronometro.isActive() == False):                                           # y se activa el cronometro si aun no lo estaba
            self.mostrar_componentes()
            self.cronometro_on_off()
            
    def test(self):
        # Test de camara
        if (self.camara_activada == False and self.imagen_estatica_cargada == False):                                                 
            self.mostrar_componentes()
            self.camara_activada = self.__controlador.activar_camara()
            self.recuperar_frame()
            
        elif (self.camara_activada == True and self.imagen_estatica_cargada == False):
            self.recuperar_frame()
            
        # Test de imagen estatica
        elif (self.imagen_estatica_cargada == True and self.camara_activada == False):
...
#%%     INVOCA METODOS DE GRAFICACION   
    ...
    def determinar_ventana_actual(self, proesamientos_aplicados, frame):                    # Se determina la ventana que esta observando
                                                                                            # el usuario, para realizar el procesamiento
                                                                                            # unicamente de la informacion de interes
        # Imagen original                                                                                    
        if (self.tabWidget_1.currentIndex() == 0 and self.camara_activada == True):                                  
            self.graficar_frame_original(frame)
        # Histograma original    
        elif (self.tabWidget_1.currentIndex() == 2): 
        ...
        if (proesamientos_aplicados > 0):
            ...
            # Imagen original
            if (self.tabWidget_2.currentIndex() == 0): 
                self.graficar_frame_procesado(frame)
...
#%%     GRAFICA DE FRAMES
    ...
    def graficar_imagen_cargada(self, imagen_cargada):
        self.escena_4.setImage(imagen_cargada)
